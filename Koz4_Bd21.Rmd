...
mapthis_F2_ <- read.cross("csvr", dir="./Koz4_Bd21", file="Koz_1st_r.csv", estimate.map=FALSE)
summary(mapthis_F2_)
```
...
main="No. genotypes by individual (#0)", ylim=c(0,500))
plot(ntyped(mapthis_F2_, what="mar"), ylab="No. typed individuals",
main="No. genotypes by marker (#0)", ylim=c(0,200))
```
  Remove missing data
...
```{r}
mapthis_F2_1 <- subset(mapthis_F2_, ind=(ntyped(mapthis_F2_)>397.8)) #397.8 >90% of all markers

nt.bymar <- ntyped(mapthis_F2_1, "mar")
todrop1 <- names(nt.bymar[nt.bymar < 157.5]) #Names of markers < 157.5 genotyped individuals.
...
, main="No. genotypes by individual (#1)", ylim=c(0,500))
plot(ntyped(mapthis_F2_1, what="mar"), ylab="No. typed individuals"
, main="No. genotypes by marker (#1)", ylim=c(0,200))

plot(ntyped(mapthis_F2_2, what="ind"), ylab="No. typed markers"
, main="No. genotypes by individual (#2)", ylim=c(0,500))
plot(ntyped(mapthis_F2_2, what="mar"), ylab="No. typed individuals"
, main="No. genotypes by marker (#2)", ylim=c(0,200))
```
```{r}
summary(mapthis_F2_)
...
saveRDS(mapthis_F2_5, file="Koz4_Bd21_mapthis_F2_5.rds")
saveRDS(mapthis_F2_6, file="Koz4-3_Bd21_mapthis_F2_6.rds")
...
```
```{r}
saveRDS(mapthis_F2_5, file="Koz4_Bd21_mapthis_F2_7.rds")
saveRDS(mapthis_F2_6, file="Koz4-3_Bd21_mapthis_F2_8.rds")
```
...
saveRDS(mapthis_F2_5, file="Koz4_Bd21_mapthis_F2_9.rds")
saveRDS(mapthis_F2_6, file="Koz4_Bd21_mapthis_F2_10.rds")
...
#entire scan
mqm_auto <- mqmscan(fill, autocofactors, pheno.col=c(1:3),　verbose = TRUE,　n.cluster=7)
#output
write.csv(mqm_auto,  file = "Koz4-3_Bd21_mqm_auto.csv")

#entire scan2
mqm_all <- mqmscanall(fill, cofactors = autocofactors, n.cluster=7)
#image output
pdf(file="Koz4_Bd21.mqmscanall(red=FW, green=nleaf blue=water).pdf")
mqmplot.multitrait(mqm_all, type="lines")
dev.off()
#image output
pdf(file="Koz4_Bd21.mqmscanall_chr=4(red=FW, green=nleaf blue=water).pdf")
...
pdf(file="Koz4_Bd21.LOD.pdf")
plot(mqm_auto.1, ylim=range(c(0, 20)))
plot(mqm_auto.2, ylim=range(c(0, 20)))
plot(mqm_auto.3, ylim=range(c(0, 20)))
dev.off()

pdf(file="Koz4_Bd21.LOD chr=4.pdf")
...
write.csv(mqmmax,  file = "Koz4_Bd21.mqmmax.csv")
```

linkage map
```{r}
#genetic map
pdf(file="Koz4-3_Bd21.genetic map.pdf")
plotMap(qtl1)
dev.off()
#genetic map
pdf(file="Koz4_Bd21.genetic map_h.pdf")
