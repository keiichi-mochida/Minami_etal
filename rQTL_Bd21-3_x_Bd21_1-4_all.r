...
setwd("C:/Users/Yoshi-RIKEN/Documents/R/Brachy analysis/2016_01_09 heat stress accumulated 1,2,3,4th")

options(digits=4)
library(qtl)

mapthis_F2_ <- read.cross("csvr",
dir="C:/Users/Yoshi-RIKEN/Documents/R/Brachy analysis/2016_01_09 heat stress accumulated 1,2,3,4th",
file="1st_4th_all_r.csv", estimate.map=FALSE)

summary(mapthis_F2_)
#individual 758, 758*0.9=682.2
...
main="0 No. genotypes by individual", ylim=c(0,800))
plot(ntyped(mapthis_F2_, what="mar"), ylab="No. typed individuals",
main="0 No. genotypes by marker", ylim=c(0,800))

#makers genotyped: > 90% of population
mapthis_F2_1 <- subset(mapthis_F2_, ind=(ntyped(mapthis_F2_)>552.6))#552.6

nt.bymar <- ntyped(mapthis_F2_1, "mar")
todrop1 <- names(nt.bymar[nt.bymar < 682.2])#682.2
...
, main="1 No. genotypes by individual", ylim=c(0,800))
plot(ntyped(mapthis_F2_1, what="mar"), ylab="No. typed individuals"
, main="1 No. genotypes by marker", ylim=c(0,800))

plot(ntyped(mapthis_F2_2, what="ind"), ylab="No. typed markers"
, main="2 No. genotypes by individual", ylim=c(0,800))
plot(ntyped(mapthis_F2_2, what="mar"), ylab="No. typed individuals"
, main="2 No. genotypes by marker", ylim=c(0,800))
...
# graph
plot(mqm_auto.1, ylim=range(c(0, 30)))
plot(mqm_auto.2, ylim=range(c(0, 30)))
plot(mqm_auto.3, ylim=range(c(0, 30)))
...
pdf(file="missing genotype data.pdf")
par(mfrow=c(1,1), las=1)
plotMissing(mapthis_F2_, main="missing genotype data")
dev.off()
...
par(mfrow=c(1,2), las=1)
plot(ntyped(mapthis_F2_, what="ind"), ylab="No. typed markers",
main="F2_ No. genotypes by individual", ylim=c(0,800))
plot(ntyped(mapthis_F2_, what="mar"), ylab="No. typed individuals",
main="F2_ No. genotypes by marker", ylim=c(0,800))
dev.off()
...
plot(ntyped(mapthis_F2_2, what="ind"), ylab="No. typed markers"
, main="2 No. genotypes by individual", ylim=c(500,700))
plot(ntyped(mapthis_F2_2, what="mar"), ylab="No. typed individuals"
, main="2 No. genotypes by marker", ylim=c(600,800))
dev.off()
